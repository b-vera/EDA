...
void leerArchivo(AVL *arbol){
    ...
    str3 = (char*)calloc(1000,sizeof(char));
...
AVL * crearAVL() {
	...
	if ( ( arbol = calloc(1, sizeof( AVL ) ) ) == NULL ) return NULL;
...
Nodo * inicializarNodo() {
	...
	if ( ( nodo = calloc(1, sizeof( Nodo ) ) ) == NULL ) {
		return NULL;
	}

    // se inicializan las cadenas de string
    nodo -> palabraEsp  = (char*)calloc(100,sizeof(char));
    nodo -> palabraOtro = (char*)calloc(100,sizeof(char));
    nodo -> defEsp	 	= (char*)calloc(1000,sizeof(char));
...
void liberarMemoria(Nodo *nodo) {
    ...
    liberarMemoria(nodo -> espL);
    liberarMemoria(nodo -> espR);
    free(nodo);
